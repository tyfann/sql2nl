...
from pre1_schema_linking import read_database_schema
...
def build_schema_linking_data(schema, question, item, turn_id, linking_type):
    ...
    source_sequence = f"<C> {column_names} | <T> {table_names} | <S> {structure_schema_str} | <Q> {item['interaction'][turn_id]['sql'].lower()}"
    target_sequence = question.lower()
...
def extract_input_and_output(example_lines, linking_type):
    inputs = []
    outputs = []
    database_schema_filename = './data/chase/tables.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sl_dataset_path", default='./data/chase_schema_linking_tag')
    parser.add_argument("--output_path", default='./dataset_post/chase_sl')
