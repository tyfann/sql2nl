...
def read_spider_split(dataset_path, table_path):
...
def preprocess_dataset(dataset, dataset_dir, output_dir, table_path):
    for session in ['train', 'dev']:
        dataset_path = os.path.join(dataset_dir, f'{session}.json')
        interaction_list = read_spider_split(dataset_path, table_path)
        write_interaction(interaction_list, session, output_dir)

    return interaction_list


def preprocess(dataset, dataset_dir, table_path, output_dir):
    # directory
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)

    # read schema
    print('Reading chase database schema file')
    ...
    preprocess_dataset(dataset, dataset_dir, output_dir, table_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", choices=('spider', 'sparc', 'cosql', 'csgsql', 'chase'), default='chase')
    ...
    preprocess(dataset, dataset_dir, table_path, output_dir)
